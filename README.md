...
In this step, we update the shell script's Shebang (`#!`) to make it working on
any machine with Nix installed.

We notice that the script now works on any system with Nix, regardless of the
installed software.
...
nix-shell \
  --expr '
    let
      repo = fetchTarball "https://github.com/drupol/demo-devshells-and-wrappers/archive/refs/heads/step3.zip";
    in
...
nix-shell \
  --expr '
    let
      repo = fetchTarball "https://github.com/drupol/demo-devshells-and-wrappers/archive/refs/heads/step4.zip";
    in
      import (if builtins.pathExists "${repo}/shell.nix" then "${repo}/shell.nix" else "${repo}/default.nix")
    ' \
  --attr go
...
nix-shell \
  --expr '
    let
      repo = fetchTarball "https://github.com/drupol/demo-devshells-and-wrappers/archive/refs/heads/step12.zip";
    in
      import (if builtins.pathExists "${repo}/shell.nix" then "${repo}/shell.nix" else "${repo}/default.nix")
    ' \
  --attr rust
...
nix-build \
  --expr '
    let
      repo = fetchTarball "https://github.com/drupol/demo-devshells-and-wrappers/archive/refs/heads/step16.zip";
    in
      import (if builtins.pathExists "${repo}/shell.nix" then "${repo}/shell.nix" else "${repo}/default.nix")
    ' \
  --attr nodejs14-bin
...
## Step 17: Using a custom package in a shell

We create a new development shell named `node14` that specifically includes our
custom `nodejs14-bin` package. This shows how to consume your own flake outputs
within your own development environments.
...
## Step 18: Adding formatting checks with `treefmt`
...
- View branch: https://github.com/drupol/demo-devshells-and-wrappers/tree/step18
- Diff:
  https://github.com/drupol/demo-devshells-and-wrappers/compare/step17..step18

## Step 19: Building a container image
...
- View branch: https://github.com/drupol/demo-devshells-and-wrappers/tree/step19
- Diff:
  https://github.com/drupol/demo-devshells-and-wrappers/compare/step18..step19

## Step 20: Adding a NixOS configuration
...
- View branch: https://github.com/drupol/demo-devshells-and-wrappers/tree/step20
- Diff:
  https://github.com/drupol/demo-devshells-and-wrappers/compare/step19..step20
