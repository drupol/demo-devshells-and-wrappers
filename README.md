...
The old `nix-shell` command still works thanks the project `flake-compat` which
provides backward compatibility for flakes. You can therefore still run:
...
## Step 4: Using an existing systems list

In this step, we will remove the custom list of systems. There is no longer any
need to hardcode and maintain the system list ourselves.
...
[`flake.parts`] leverage Nix's inherent modularity, bringing the concept of
modules to flakes. This allows you to break down complex configurations into
smaller, more manageable and reusable pieces.
...
```shell
❯ nix-build --attr packages.x86_64-linux.nodejs14-bin
